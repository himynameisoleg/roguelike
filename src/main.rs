...
fn place_objects(room: Rect, map: &Map, objects: &mut Vec<Object>) {
    ...
    let num_monsters = rand::thread_rng().gen_range(1, MAX_ROOM_MONSTERS + 1);
...
fn move_towards(id: usize, target_x: i32, target_y: i32, map: &Map, objects: &mut [Object]) {
    // vectore from this object to the target and dist
    let dx = target_x - objects[id].x;
    let dy = target_y - objects[id].y;
    let distance = ((dx.pow(2) + dy.pow(2)) as f32).sqrt();

    // normalize it to len 1 (preserving direction), then round it and convert to int so the
    // movement is restricted to the map grid
...
fn render_all(tcod: &mut Tcod, game: &mut Game, objects: &[Object], fov_recompute: bool) {
    if fov_recompute {
        // recomputer FOV if needed (the player moved)
        let player = &objects[PLAYER];
        tcod.fov
            .compute_fov(player.x, player.y, TORCH_RADIUS, FOV_LIGHT_WALL, FOV_ALGO);
    }
    ...
    for y in 0..MAP_HEIGHT {
        for x in 0..MAP_WIDTH {
            ...
            if visible {
                // since its visiblee, explore it
                *explored = true;
            }
            if *explored {
                // show explored tiles only (any visible tilee is explored already)
                tcod.con
...
fn main() {
    ...
    // this list of objects with just the player
    let mut objects = vec![player];

    let mut game = Game {
        // generate map (at this point its not drawn on screen)
        map: make_map(&mut objects),
    };

    // populate the FOV map according to generated map
    for y in 0..MAP_HEIGHT {
        for x in 0..MAP_WIDTH {
            tcod.fov.set(
                x,
    ...
    // force FOV "recomputer" first time through the game loop
    ...
    while !tcod.root.window_closed() {
        ...
        // handle keys and exit games if needed
